...
% P175. Q1
% =========================================
\begin{problem}{P175. Q1}
    试编写 MATLAB 函数实现 Newton 插值，要求能输出插值多项式。对函数
    \[
        f(x)=\frac{1}{1+4x^2}
    \]
    在区间 $[-5,5]$ 上实现 $10$ 次多项式插值。
    \begin{enumerate}[1.]
        \item 输出插值多项式；
        \item 在区间 $[-5,5]$ 内均匀插入 $99$ 个节点，计算这些节点上函数 $f(x)$ 的近似值，并在同一张图中画出原函数和插值多项式的图形；
        \item 观察龙格现象，计算插值函数在各节点上的误差，并画出误差图。
    \end{enumerate}
\end{problem}
...
\begin{solution}
    ...
    差商表函数 \texttt{chap-5/divided\_difference.m}：
...
\end{lstlisting}
    ...
    Newton 插值函数 \texttt{chap-5/newtonInterp.m}：
...
\end{lstlisting}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.88\textwidth]{../../bit-numerical-analysis-hw/chap-5/assets/P175_Q1.png}
    \end{figure}

    \textbf{实验结果分析}

    从图中可以清楚观察到龙格现象：在区间中心附近，插值多项式与原函数吻合较好；而在区间两端尤其接近 $\pm 5$ 时，多项式出现明显振荡，误差迅速放大。这说明等距节点上的高次多项式插值并不总能提升精度，反而可能在端点区域恶化逼近效果。若要减弱这种现象，可考虑改用分段低次插值或样条插值。
\end{solution}
...
% =========================================
\begin{problem}{补充代码}
    下面整理第 5 章作业与实验中用到的补充程序及运行结果。
\end{problem}

\begin{solution}
    \textbf{\texttt{lagrangeInterp\_disp.m}}
